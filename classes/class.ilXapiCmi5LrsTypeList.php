...
class ilXapiCmi5LrsTypeList
{
    ...
    public static function getTypesData($a_extended = false, $a_availability = null)
    {
        ...
        $query = "SELECT * FROM " . ilXapiCmi5LrsType::getDbTableName();
    ...
    {
        global $ilDB;
        $query = "SELECT COUNT(*) counter FROM " . ilXapiCmi5LrsType::getDbTableName() . "
					WHERE availability = " . $ilDB->quote(ilXapiCmi5LrsType::AVAILABILITY_CREATE, 'integer');
        $res = $ilDB->query($query);
        $row = $ilDB->fetchObject($res);
        return $row->counter;
    }
